...
4. **扩展名默认锁定，且不显示在输入框里**（连灰色后缀也不放，由输入框下方的提示行写明保留的扩展名）；勾选最底部的“允许修改扩展名”后，扩展名才拼进输入框一起编辑。
5. 输入框下方并排两个按钮：**发送**（丢进系统分享面板再分享）和**保存到下载**（Android 10+ 走 MediaStore 直写、Android 7–9 走 SAF 选择器，同样零权限）。
6. 发送或保存成功后 **Toast 提示并自动关闭主界面**，避免用户在分享面板里又把文件选回本应用。
7. 底部固定一条红色警示语，提醒发送后在分享面板选择其它应用接收、勿再选本应用，防止无限循环。
...
| 锁定时**完全隐藏**扩展名，勾选后才把扩展名拼进输入框（全程不用 TextWatcher 硬顶） | 未勾选时输入框里只有“基础名”，改名焦点完全放在名字上；保留的扩展名由下方提示行写明（`锁定为 .png`）。用户仍可以在名字里随便用点号（`my.file.v2` 不会被误切），勾选后扩展名变成普通可编辑文本。 |
| 保存到下载：10+ 走 MediaStore、7–9 走 SAF，全程零权限 | 写自己新建进 `Downloads` 的文件在 scoped storage 下不需要任何权限，保住“零权限”卖点；老系统退回系统选择器，只多一次确认，也不用申请 `WRITE_EXTERNAL_STORAGE`（那会让 QA 体检的权限门禁失败）。 |
| 只有能读到文件大小时才走 `FileChannel.transferTo`，并按字节数校验；拿不到大小（pipe 型 Provider）或校验不符就退回 64 KiB 流复制 | `transferTo` 允许“少传”甚至返回 0，只有在 `statSize` 可校验时才敢用零拷贝；pipe 型 Provider 走流复制，两条路都不会把不完整的文件当成成功。 |
...
缓存文件放在 `cacheDir/shared/<会话>/`，系统可以回收；App 自己也会清理，规则按“有没有交出去”分成两种：

| 会话状态 | 什么时候删 | 为什么 |
| --- | --- | --- |
| **未交付**（取消、直接杀进程、复制失败） | 超过 10 分钟 | 没有任何第三方握有它的 URI，留着只会白占空间；10 分钟是给界面重建和重试留的余量 |
| **已交付**（发送过 / 保存过） | 从**交付时刻**起 24 小时 | 对方的 `content://` URI 在它自己的任务存活期间一直有效，可能还在异步上传，用户也可能稍后再次转发 |

清理全程在 IO 线程（老版本在主线程同步删，攒几个大文件就会卡冷启动），并且**绝不会碰当前界面正在用的会话**：进程内的活会话登记 + `cacheDir/shared-state/` 里的交付标记（跨进程持久化）一起把这件事钉死。触发点是冷启动（强制）+ 复用界面时收到新分享 + 新建复制会话，后两者按 10 分钟节流。
...
- 新文件名按 UTF-8 **字节数**截断（上限 240 字节，中文约 78 字，扩展名始终完整保留），因为 `ext4/f2fs` 的单个文件名上限是 255 字节而不是 255 个字符。
